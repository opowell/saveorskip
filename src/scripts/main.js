...
var idbApp = (function() {
  ...
  function getByPrice() {
    ...
    dbPromise
      ...
      .then(function showRange(cursor) {
        ...
        s += '<h2>Price - ' + cursor.value.price + '</h2>';
        s += printFields(cursor.value);
  ...
  function printFields(item) {
    ...
    for (var field in item) {
      s += field + '=' + item[field] + '<br/>';
    }
    s += '</p>';
    return s;
  }

  function addItems(items, storeName) {
  ...
  function showOrders() {
    var s = '';
    getOrders()
      .then(function showRange(items) {
        for (let i in items) {
          let item = items[i];
          console.log('Cursored at:', item.name);
          s += '<h2>' + item.name + '</h2>';
          s += printFields(item);
        }
        return true;
  ...
  function openTestPage() {
    window.open('http://localhost:8080/test/test.html', '_blank');
  }
