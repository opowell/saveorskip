...
export async function loadProfile(payload) {
  ...
  profile['Logs'] = await getNumResults({
    ...
    filters: [
      ...
      { field: 'objectKeys', operator: 'eq', value: profileId },
    ],
  });
  return profile;
}
...
export async function fetchRegularProfiles() {
  ...
  for (let i = 0; i < values.length; i++) {
    ...
    values[i].sources = await db.countFromIndex(STORE_SOURCES, STORE_SOURCES_CONSUMERID, values[i].id);
...
export async function fetchProfiles() {
  ...
  for (let i = 0; i < values.length; i++) {
    values[i].links = await db.countFromIndex(STORE_LINKS, STORE_LINKS_PROFILEID, values[i].id);
    values[i].sources = await db.countFromIndex(STORE_SOURCES, STORE_SOURCES_CONSUMERID, values[i].id);
    values[i].logs = await db.countFromIndex(STORE_LOGS, STORE_LOGS_PROFILEID, ['Profile', values[i].id]);
    console.log('pushing ', values[i]);
    profiles.push(values[i]);
...
async function getCursor(query) {
  ...
  let cursor = await index.openCursor(keyRng);
  out = cursor;
  return out;
}
