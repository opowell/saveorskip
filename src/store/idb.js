...
export async function loadPopupProfile() {
  ...
  const db = await dbPromise;
  let profile = await db.get(STORE_PROFILES, profileId);
  dispatchToStores('setPopupProfile', profile);
}
...
export async function storeProfile(profile, overwriteProps) {
...
export async function saveOrSkipSource({ source, targetId, action }) {
  ...
  if (source.pointsSave != null && action === 'save') {
    sourceConnection.points = source.pointsSave;
  }
  if (source.pointsSkip != null && action === 'skip') {
    sourceConnection.points = source.pointsSkip;
  }
...
export async function addProfile(profile) {
  if (profile.defaultAction == null) {
    profile.defaultAction = 'save';
  }
