...
export async function loadProfile(payload) {
  ...
  profile['Logs'] = await db.countFromIndex(STORE_LOGS, INDEX_LOGS_PROFILEID, ['Profile', profile.id]);
  return profile;
}
...
export async function fetchProfiles() {
  ...
  for (let i = 0; i < values.length; i++) {
    values[i].links = await db.countFromIndex(STORE_LINKS, INDEX_LINKS_PROFILEID, values[i].id);
    values[i].sources = await db.countFromIndex(STORE_SOURCES, INDEX_SOURCES_CONSUMERID, values[i].id);
    values[i].logs = await db.countFromIndex(STORE_LOGS, INDEX_LOGS_PROFILEID, ['Profile', values[i].id]);
...
export async function storeLinkSource(source) {
  ...
  if (source.source.autoGenerated == null) {
    source.source.autoGenerated = true;
  }
...
async function getCursorFn(lowerBounds, upperBounds, queryName) {
  let out = null;
  const db = await dbPromise;
  let tx = await db.transaction(STORE_LINKS);
  let objStore = await tx.objectStore(STORE_LINKS);
  let index = await objStore.index(queryName);
  let keyRng = IDBKeyRange.bound(lowerBounds, upperBounds);
  let cursor = await index.openCursor(keyRng, 'prev');
  out = cursor;
  await tx.done;
  return out;
...
export async function getCursor(storeName, query) {
  console.log('getting ' + storeName + ' by ', query);
  let cursor;
  try {
    cursor = await getCursorFn(query.lowerBounds, query.upperBounds, query.name);
  } catch (e) {
    if (e.name === 'NotFoundError') {
      createIndex(storeName, query);
      cursor = await getCursorFn(query.lowerBounds, query.upperBounds, query.name);
    }
  }
  return cursor;
...
async function createIndex(storeName, query) {
  setDBVersion(getDBVersion() + 1);
  await openDB(DB_NAME, getDBVersion(), {
    async upgrade(db, oldVersion, newVersion, transaction) {
      const store = db.transaction(storeName).objectStore(storeName);
      store.createIndex(query.name, query.keyPath);
    },
  });
}
  ...
  if (filters != null) {
    for (let i = 0; i < filters.length; i++) {
      ...
      if (filter.operator === 'eq') {
        ...
        query.keyPath.push(filter.name);
